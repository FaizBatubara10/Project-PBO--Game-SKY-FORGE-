...
public class p3 extends Actor implements Scorable, Shootable
{
    ...
    public p3()
    {
        move = new GreenfootImage("Move(3).png");

        destroy = new GreenfootImage[5];
        for (int i = 0; i < destroy.length; i++) {
            destroy[i] = new GreenfootImage("Destroyed(3-" + (i + 1) + ").png");
        }

        shoot = new GreenfootImage[4];
        for (int i = 0; i < shoot.length; i++) {
            shoot[i] = new GreenfootImage("Attack_1(3-" + (i + 1) + ").png");
        }

        damage = new GreenfootImage[7];
        for (int i = 0; i < damage.length; i++) {
            damage[i] = new GreenfootImage("Damage(3-" + (i + 1) + ").png");
        }
    ...
    public void shoot()
    {
        ...
        if (shootDelay > 20)
        {
            ...
            l3 peluruAtas = new l3();
            l3 peluruBawah = new l3();
            getWorld().addObject(peluruAtas, getX() - 20, getY() + 5);
            getWorld().addObject(peluruBawah, getX() - 20, getY() - 5);
            shootDelay = 0;
        }
    }
    ...
    public int getScoreValue() {
        return 150; // nilai poin untuk musuh p3
    }
}
